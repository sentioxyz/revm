...
// imports
use crate::{
    instruction_context::InstructionContext, interpreter_types::*, CallInput, Gas, GasTable, Host,
...
impl<EXT: Default> Interpreter<EthInterpreter<EXT>> {
    ...
    pub fn clear(
        ...
        *gas = if sentio_config.ignore_gas_cost() {
            Gas::new(gas_limit, true)
        } else {
            Gas::new_with_regular_gas_and_reservoir(gas_limit, reservoir_remaining_gas)
        };
...
impl<IW: InterpreterTypes> Interpreter<IW> {
    ...
    pub fn run_plain<H: Host + ?Sized>(
        ...
        if let Some(selector) = self.function_selector() {
            if let Some(mock_function) =
                sentio_config.get_mock_function(self.input.target_address(), selector)
            {
                trace!(
                    "mock function: {} on {}",
                    selector,
                    self.input.target_address()
                );
    ...
    pub fn function_selector(&self) -> Option<Selector> {
        ...
        match input {
            CallInput::Bytes(bytes) => Selector::try_from(&bytes[..Selector::len_bytes()]),
